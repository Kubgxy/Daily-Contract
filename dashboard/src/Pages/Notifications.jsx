...
const Notifications = () => {
  ...
  const loadNotifications = async () => {
    try {
      setIsLoading(true)
      const response = await axios.get(`http://localhost:3000/api/data/getAllNotifications`, { withCredentials: true })
  ...
  const handleSendNotification = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:3000/api/data/addNotification", newNotification,{ withCredentials: true })
  ...
  const handleDeleteNotification = async (id) => {
    try {
      setIsLoading(true)
      await axios.delete(`http://localhost:3000/api/data/deleteNotification/${id}`, { withCredentials: true })
    ...
    try {
      ...
      await Promise.all(
        selectedNotifications.map((id) =>
          axios.delete(`http://localhost:3000/api/data/deleteNotification/${id}`, { withCredentials: true }),
        ),
      )
    ...
    try {
      ...
      if (response.status === 200) {
        setNotifications((prev) =>
          prev.map((notification) => (notification._id === editingId ? response.data.data : notification)),
        )
        setCurrentEntry({ category: "", message: "", details: "" })
        setEmployeeId("")
        setEditingId(null)
  ...
  const filteredNotifications = notifications.filter(
    (notification) =>
      notification.employee_id.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <motion.div className="p-6 max-w-7xl mx-auto" initial="hidden" animate="visible" variants={containerVariants}>
        ...
        <motion.div className="lg:col-span-1" variants={itemVariants}>
          <div className="bg-white dark:bg-dark-800 rounded-xl shadow-md p-6">
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={editingId ? handleUpdateNotification : handleSendNotification}
                disabled={isLoading}
        ...
        <motion.div className="lg:col-span-2" variants={itemVariants}>
          <div className="bg-white dark:bg-dark-800 rounded-xl shadow-md p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-white">รายการแจ้งเตือน</h2>
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                ...
                <button
                  ...
                  className={`btn btn-danger h-10 ${selectedNotifications.length === 0 || isLoading ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                      ...
                      {currentNotifications.length > 0 ? (
                        currentNotifications.map((notification) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 whitespace-nowrap">
                              <span
                                className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getCategoryColor(notification.category)}`}
                              >
                        ...
                        <tr>
                          <td colSpan="6" className="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
                            {searchTerm ? "ไม่พบการแจ้งเตือนที่ตรงกับคำค้นหา" : "ไม่มีการแจ้งเตือน"}
                          </td>
                        </tr>
                      )}
                    </tbody>
      ...
      {successMessage && (
        <div className="fixed bottom-4 right-4 z-50 max-w-md">
          <div
            className={`bg-white dark:bg-dark-800 rounded-lg shadow-lg border ${successMessage.includes("Error") ? "border-danger-500" : "border-green-500"} overflow-hidden`}
          >
